...
const Quiz = props => {
  ...
  const handleAnswerOnClick = index => {
    if (index === correct) {
      setIsCorrect(true);
      return;
    }
  ...
  return (
    <div className="mcqs-frontend">
      <p>{question}</p>
      <ul>
        {answers.map((answer, index) => {
          return (
            <li key={index} 
              onClick={isCorrect ? undefined : () => handleAnswerOnClick(index)}
              className={(isCorrectDelayed && index === correct ? 'no-click' : '') + (isCorrectDelayed && index !== correct ? 'fade-incorrect' : '')}
            >
              {isCorrectDelayed && index === correct && (
